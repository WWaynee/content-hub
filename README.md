...
- **可配置检索策略链（P14）**：纯向量检索（默认）→ BM25(2-gram)+向量混合（RRF 融合）→ +Rerank 精排 → +LLM 查询扩展，全部策略开关默认关闭、主流程零风险；配套自建评测基准（20 query × 3 难度），实测 Recall@5 提升 +30pp、NDCG@5 +47%
- **多租户隔离**：租户间数据完全隔离（含向量检索隔离）

![界面截图](docs/images/screenshot-1.png)

![界面截图](docs/images/screenshot-2.png)
