...
- [ ] 向量化入库 worker（RabbitMQ document_parse 异步队列 + cmd/worker 进程 + 上传投递改异步）
...
### 7B · 初始化 React 前端工程（进行中）
- [ ] 初始化 React + Vite + TypeScript 工程（web/ 目录）
- [ ] 配置 axios / 路由 / 基础布局（左侧导航：工作区 | 知识库）

### 7C · 前端页面（待做）
- [ ] 登录/注册
- [ ] 首页工作区列表（倒序 + 标题/标签/平台/状态检索）+ 新建/删除
- [ ] 工作区详情：左侧「需求 | 稿件」两阶段 + 对话面板（target 锚点）+ 导出
- [ ] 知识库：公有/私有、左右分栏（左管理 + 右问答对话）、目录树上传/覆盖/删除、预览/下载
- [ ] 反代/构建配置
