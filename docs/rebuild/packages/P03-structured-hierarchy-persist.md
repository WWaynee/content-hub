...
## 🧭 回顾（面试/复盘用）：P03 到底修了什么

### 一句话
**稿件在数据库里只保存了"一句话列表"，丢掉了"章节/段落"结构——前端/后续模块拿不到这篇稿分几章、每章几段，只能看到平铺句子。**

### 原先在什么场景会发生
| 场景 | 触发 | 后果 |
|------|------|------|
| 生成多章多段的长稿 | `agent.Article` 明明分 `Sections→Paragraphs→Sentences` 三层，落库遍历却没写两个结构字段，只写全局平铺号 | DB 里 section/paragraph 恒 0 → 所有句子像挤在一层 |
| 想"按段看稿 / 按段改成渲染 UI" | 第一步都要问"这篇分几段、每句归哪段" | DB 答不出，只能去解析一段临时拼出的 `full_content` markdown |
| 多次修订/追加后 | 重建句子行仍按"第 N 句"平铺重写 | 把已丢的结构固化，损失不可逆 |

> 本质：结构是稿件身份的一部分，但存储时被塞进了"给人展示的字符串"，没进"给程序当数据的表"。评论/溯源/前端想按结构做事，只能拿到一段给人读的文本——做不到或靠猜。

### 宏观方案（精神，做非代码）
核心一句话：**把结构当成与文本同等的一等数据落库，让写与读都沿同一套三级坐标 `(section, paragraph, sentence)` 走，而不是依赖临时字符串。**

1. 写时分离“顺序编号”与“结构编号”：每句同时有“本快照第几句”(仅内部绑定引用)与“属于第几节第几段的第几句”(落库)。存结构号，不让顺序器冒充结构字段。
2. 约定统一的读法：所有读取一律按结构化坐标排序 → “读回来即清晰分层”，而非“存了但读出来仍乱”。
3. 写生/追加/修订保留并推进结构：只改一句文本不重算顺序把段落拍平；改句不换归属，追加=新增一段(段内 max+1)，让结构稳定演化而非反复归零。
4. 旧数据无歧义出口：历史无结构的老稿不靠猜全文猜标题，明确“按线性返回+标记无结构”，诚实降级不伪造结构。

一句话精神：**结构不是藏在 markdown 里给眼睛看，而是在 DB 里作为 (section, paragraph, sentence) 坐标真实留存，读写两端沿同一坐标走——这是 P08(稳定句/调结构) 与 P11(展示/改稿 UI) 能工作的前提。**
