# 稿件协作多 Agent 化重建方案
> ⚠️ 本文是**完整 RFC/理由书(全文保留)**。要按「改造顺序 + 点位」开工/验收，请在 `docs/rebuild/README.md` 看**总索引/依赖矩阵/验收表**，并到 `docs/rebuild/packages/` 使用**每阶段一份、可独立执行并验收**的工作包。下方原文按 rev-1→rev-4 叙述结论与权衡，不再承担"直接开工不卡壳"的职责。
